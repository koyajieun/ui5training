sap.ui.define([
     ...
     */
    function (BaseController, JSONModel, Filter, FilterOperator, daumPost, Fragment, Dialog, MessageToast, History, UIComponent,MessageBox) {
        "use strict";

        return BaseController.extend("sap.sync.ui5training.controller.View1", {
            ...
            getRouter: function () {
                ...
                //이 컨트롤러(뷰)를 관장하는 라우터를 찾아올려고 이 함수를 만듬
                //sap.ui.core.UIComponent.getRouterFor(oControllerOrView) // this or this.getView()
            ...
            onNavToView2: function () {
                ...
                // console.log("tab;e",iIndex);
                var oViewModel = this.getView().getModel();
                var sPath = "/students/"+iIndex+"/name"; //students/0/name
                var sName = oViewModel.getProperty(sPath);

                if (iIndex === undefined) { //table에서 암것도 선택안했을때
                    //messagebox 선택하라고 사용자에게 경고장
                    this.onWarning2MessageBoxPress();
                } else {
                    this.getRouter().navTo("RouteView2", {name:sName});
                }
                //  manifest.json route의 name
            },
            onNavToView3: function () {

                // var sName = oViewModel.getProperty(sPath);
                this.getRouter().navTo("RouteView3");
                //navto는 그냥 라우터한테 어디로 가달라는 요청(어디로가라)

            },
            onWarning2MessageBoxPress: function () {
                var sText = "목록에서 학생한명만 선택해줘"
                MessageBox.warning(sText,
                {
                    actions: [MessageBox.Action.OK],
                    emphasizedAction: MessageBox.Action.OK //강조할버튼 하이라이트
                }
                );
            }
